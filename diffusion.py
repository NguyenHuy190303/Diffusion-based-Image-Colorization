...
class ColorDiffusion(nn.Module):
    ...
    def set_new_noise_schedule(self, device):
        """
        Initialize noise schedule and precompute diffusion-related constants.
        """
        ...
        gammas = np.cumprod(alphas, axis=0)  # Cumulative product of alphas
        gammas_prev = np.append(1.0, gammas[:-1])  # Shifted gammas for previous timestep

        # Register buffers for diffusion constants
        ...
        # Posterior variance and coefficients
    ...
    def predict_start_from_noise(self, y_t, t, noise):
        """
        Predict the starting image (y_0) from noisy image (y_t) and noise.
        """
        return (
    ...
    def p_mean_variance(self, y_t, t, clip_denoised: bool, y_cond=None):
        """
        Compute the predicted mean and variance for the reverse diffusion process.
        """
    ...
    def q_sample(self, y_0, sample_gammas, noise=None):
        """
        Sample noisy images from the forward diffusion process.
        """
        noise = noise if noise is not None else torch.randn_like(y_0)
        return (
            sample_gammas.sqrt() * y_0 +
            (1.0 - sample_gammas).sqrt() * noise
        )
    ...
    def restoration(self, y_cond, y_t=None, sample_num=8):
        """
        Perform the full reverse diffusion process to restore the colorized image.
        """
        b, _, h, w = y_cond.shape
        y_t = y_t if y_t is not None else torch.randn((b, 2, h, w)).to(y_cond.device)
        ret_arr = y_t
    ...
    def forward(self, y_0, y_cond=None, noise=None):
        """
        Forward pass for training. Adds noise to the input and computes the loss.
        """
        b, *_ = y_0.shape
